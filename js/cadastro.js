...
const Cadastro = {
  ...
  async iniciarCamera() {
    /* ★ câmera primeiro, com diagnóstico completo em português */
    try {
      this.stream = await Camera.abrir({ facingMode: "user", width: { ideal: 640 }, height: { ideal: 480 } });
    } catch (e) {
      console.error(e);
      const overlay = document.getElementById("poseOverlay");
      overlay.style.display = "";
      overlay.innerHTML = Camera.painelErro(e?.cameraInfo || Camera._traduzir(e));
      const btnRetry = document.getElementById("camRetryBtn");
      if (btnRetry) btnRetry.onclick = () => this.iniciarCamera();
    ...
    const overlay = document.getElementById("poseOverlay");
    overlay.style.display = "";
    overlay.innerHTML = '<div class="pose-instr" id="poseInstr">Carregando IA facial…</div>';
    ...
    /* IA facial carrega DEPOIS — se falhar, a câmera continua no ar */
    try {
      if (!Face.pronto) await Face.carregar();
    } catch (e) {
      console.error(e);
      document.getElementById("poseInstr").textContent = "IA não carregou — verifique a internet e pare/ative a câmera de novo";
      UI.toast("Câmera ligada, mas a IA facial falhou. Verifique a internet e tente de novo.", "err", 6000);
      return;
    }
  ...
  pararCamera() {
    ...
    const overlay = document.getElementById("poseOverlay");
    overlay.innerHTML = '<div class="pose-instr" id="poseInstr">Câmera parada</div>';
  },
